...
module.exports = {
  ...
  editForm(req, res) {
    ...
    Promise.all(queryAll).then(([service, categories]) => {
  ...
  contactAdd(req, res) {
    ...
    const service = Service.findById({ _id: id }).then(service => {
  ...
  contactEditForm(req, res) {
    ...
    Service.findOne({ "subcontact._id": id }).then(service => {
  ...
  locationUpdate(req, res) {
    ...
    Service.findByIdAndUpdate({ "locations._id": id }).then(service => {
      res.send(service);
  ...
  includeDelete(req, res) {
    const id = req.params.id;
    Service.findOne({ "included._id": id }).then(include => {
      include.included.pull({ _id: id });
      include.save().then(service => {
        res.render("admin/service-edit", { service: service });
      });
    });
  },
  ...
  excludeDelete(req, res) {
    const id = req.params.id;
    Service.findOne({ "excluded._id": id }).then(exclude => {
      exclude.excluded.pull({ _id: id });
      exclude.save().then(service => {
        res.render("admin/service-edit", { service: service });
      });
    });
  }
